...
public class DynmicStringPatcherTranspiler
{
    ...
    public static IEnumerable<CodeInstruction> ReplaceWithTranspiler(IEnumerable<CodeInstruction> instructions)
    {
        ...
        foreach (var replacement in ContractsToApply)
        {
            ...
            if (!foundReplacement)
                DynamicStringPatcherPlugin.Logger.LogError($"Skipped Contract: {isDangerous} :: {replacement.Type}.{replacement.Method}.{preparedRaw}.{preparedTrans}");
        }
    ...
    public static HarmonyMethod CreateTranspilerMethod(DynamicStringContract[] contractsToApply)
    {
        ...
        var methodInfo = typeof(DynmicStringPatcherTranspiler).GetMethod("ReplaceWithTranspiler",
